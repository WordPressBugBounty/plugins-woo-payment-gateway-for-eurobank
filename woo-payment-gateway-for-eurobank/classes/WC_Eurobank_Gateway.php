...
class WC_Eurobank_Gateway extends \WC_Payment_Gateway {
    ...
    public const ENCRYPTION_KEY_LENGTH = 32; /* 32-byte NONCE_KEY slice; cipher stays AES-128 — matches the released 2.0.x storage format. Do NOT switch to AES-256 (breaks stored secrets). See .docs/adr/0001. */

    protected string $notify_url;
    protected ?string $redirect_page_id;
    protected string $eb_PayMerchantId;
    protected string $eb_PayMerchantKey;
    protected int $pb_installments;
    protected string $test_mode;
    protected string $eb_installments_variation;
    protected string $eb_transactionType;
    protected string $eb_render_logo;
    protected string $eb_enable_log;
    protected string $eb_order_note;
    ...
    public function admin_options() {
        ...
        echo '<p>' . __( static::PLUGIN_TITLE . ' allows you to accept payment through various channels such as Maestro, Mastercard, AMex cards, Diners  and Visa cards.', static::TEXT_DOMAIN ) . '</p>';
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'test_mode'                 => array(
                ...
                'description' => __( 'Enable or disable the test environment.', static::TEXT_DOMAIN ),
                'desc_tip'    => true,
                'default'     => 'no',
            ),
            'title'                     => array(
                ...
                'desc_tip'    => false,
                'default'     => __( 'Credit card via Eurobank', static::TEXT_DOMAIN ),
            ),
            'description'               => array(
                ...
                'default'     => __( 'Pay Via Eurobank: Accepts  Mastercard, Visa cards and etc.', static::TEXT_DOMAIN ),
            ),
            'eb_render_logo'            => array(
                'title'       => __( 'Display the logo of EuroBank', static::TEXT_DOMAIN ),
                'type'        => 'checkbox',
                'description' => __( 'Enable to display the logo of EuroBank next to the title which the user sees during checkout.', static::TEXT_DOMAIN ),
                'default'     => 'yes',
            ),
            'eb_PayMerchantId'          => array(
                'title'       => __( 'Eurobank Merchant ID', static::TEXT_DOMAIN ),
                'type'        => 'text',
                'description' => __( 'Enter Your Eurobank Merchant ID', static::TEXT_DOMAIN ),
                'default'     => '',
                'desc_tip'    => true,
            ),
            'eb_PayMerchantKey'         => array(
                'title'       => __( 'Eurobank Merchant KEY', static::TEXT_DOMAIN ),
                'type'        => 'password',
                'description' => __( 'Enter Your Eurobank Merchant KEY', static::TEXT_DOMAIN ),
                'default'     => '',
                'desc_tip'    => false,
            ),
            ...
            'pb_installments'           => array(
                ...
                'description' => __( '1 to 24 Installments,1 for one time payment. You must contact Eurobank first<br /> If you have filled the "Max Number of installments depending on the total order amount"', static::TEXT_DOMAIN ),
            ),
    ...
    public function get_option( $key, $empty_value = null ) {
        ...
        if ( $key === 'eb_PayMerchantKey' ) {
            $decrypted    = self::decrypt( base64_decode( $option_value ), substr( NONCE_KEY, 0, self::ENCRYPTION_KEY_LENGTH ) );
    ...
    public function payment_fields() {
        ...
        if ( $max_installments > 1 ) {
            $doseis_field = '<p class="form-row "><label for="' . esc_attr( $this->id ) . '-card-doseis">' .
                __( 'Choose Installments', static::TEXT_DOMAIN ) . ' <span class="required">*</span></label><select id="' .
    ...
    public function validate_eb_PayMerchantKey_field( $key, $value ) {
        $encrypted = self::encrypt( $value, substr( NONCE_KEY, 0, self::ENCRYPTION_KEY_LENGTH ) );
    ...
    public function generate_eurobank_form( $order_id ) {
        ...
        wc_enqueue_js( '
             $.blockUI({
             message: "' . esc_js( __( 'Thank you for your order. We are now redirecting you to Eurobank to make payment.', static::TEXT_DOMAIN ) ) . '",
             baseZ: 99999,
             overlayCSS:
             {
             background: "#fff",
        ...
        // A09-2021 Security Logging and Monitoring Failures fix:
        ...
        // }
        $this->safe_log( '---- Eurobank Transaction digest -----' );
        $this->safe_log( 'Data: ', $form_data );
        $this->safe_log( 'Digest: ', $digest );
        $this->safe_log( '---- End of Eurobank Transaction digest ----' );
    ...
    public function process_payment( $order_id ) {
        $order  = new \WC_Order( $order_id );
        $k      = esc_attr( $this->id ) . '-card-doseis';
        $doseis = ! empty( $_POST[ $k ] ) ? (int) $_POST[ $k ] : 0;
    ...
    public function receipt_page( $order_id ) {
        echo '<p>' . __( 'Thank you - your order is now pending payment. You should be automatically redirected to Eurobank Paycenter to make payment.', static::TEXT_DOMAIN ) . '</p>';
    ...
    public function check_eurobank_response() {
        ...
        // }
        $this->safe_log( '---- Eurobank Response -----', $_POST );
        $this->safe_log( '---- End of Eurobank Response ----' );
        ...
        if ( $orderid === '' ) {
            ...
            $this->safe_log( "Eurobank: something went wrong with order id" );
        ...
        // Build the response digest deterministically: the exact field order the bank signs,
        // over raw (unslashed) values — sanitizing or reordering here yields a mismatched
        // digest and a wrongly-failed payment. Mirrors main's reconstruction (commit 6c74c96).
        ...
        // Keep encoding normalization consistent with the request-side hashing.
        ...
        if ( $status === 'CAPTURED' || $status === 'AUTHORIZED' ) {
            ...
            if ( $order->get_status() === 'processing' ) {
                $order->add_order_note( __( 'Payment Via Eurobank<br />Transaction ID: ', static::TEXT_DOMAIN ) . $paymentRef );
                ...
                if ( $this->eb_order_note === 'yes' ) {
                    $order->add_order_note( __( 'Payment Received.<br />Your order is currently being processed.<br />We will be shipping your order to you soon.<br />Eurobank Transaction ID: ', static::TEXT_DOMAIN ) . $paymentRef, 1 );
                }
                ...
                if ( $this->eb_order_note === 'yes' ) {
                    $order->add_order_note( __( 'Payment Received.<br />Your order is now complete.<br />Eurobank Transaction ID: ', static::TEXT_DOMAIN ) . $paymentRef, 1 );
                }
            }
    ...
    protected function updateStatus( $client_message, $message_type, $order, $status = null, $message = null ) {
        $eb_message = array( 'message' => __( $client_message, static::TEXT_DOMAIN ), 'message_type' => $message_type );
    ...
    private static function encrypt( $message, $key ) {
        if ( mb_strlen( $key, '8bit' ) !== self::ENCRYPTION_KEY_LENGTH ) {
            throw new \Exception( "Needs a " . ( self::ENCRYPTION_KEY_LENGTH * 8 ) . "-bit key! Got " . mb_strlen( $key, '8bit' ) );
        }
    ...
    private function sanitize_log_data( $data ) {
        if ( is_string( $data ) ) {
            // Replace merchant key with redacted text
            $data = str_replace( $this->eb_PayMerchantKey, '[REDACTED]', $data );
        } elseif ( is_array( $data ) ) {
            // Sanitize sensitive keys in arrays
            $sensitive_keys = [ 'eb_PayMerchantKey', 'PayMerchantKey', 'digest' ];
